// frontend/src/pages/hr/HRPipeline.tsx
import { useMemo, useState } from 'react';
import { GripVertical, MoreHorizontal, User, Lock } from 'lucide-react';
  ...
  const handleDrop = (status: ApplicationStatus) => {
    ...
    if (draggedId) {
      ...
      if (candidate && candidate.status !== status) {
        ...
        toast.success(
          `${candidate.fullName} moved to "${pipelineStages.find((s) => s.key === status)?.label}"`
        );
      }
    }
    setDraggedId(null);
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            {isManagement
              ? 'Read-only view of candidates across all stages.'
        ...
        {columns.map((col) => (
          <div
            ...
            className={cn(
              ...
              dragOverStage === col.key && canWrite
                ...
                col.candidates.map((c) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        if (canWrite) setSelectedCandidateId(c.id);
                      }}
                      className={cn(
                        'group rounded-lg border bg-card p-3 shadow-sm transition-all',
                        canWrite && 'cursor-pointer hover:shadow-md hover:border-accent/40',
                    ...
                    >
                      ...
                      {(vacancy || company) && (
                        <div className="mt-2 border-t border-border/40 pt-2">
                          {vacancy && (
                            <p className="truncate text-xs font-medium">{vacancy.title}</p>
                          )}
                          {company && (
                            <p className="truncate text-xs text-muted-foreground">
                              {company.name}
